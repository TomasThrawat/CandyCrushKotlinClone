...
class GameView(context: Context) : View(context) {
    ...
    private fun drawBlueberry(c:Canvas,x:Float,y:Float,r:Float){c.drawCircle(x,y,r*.88f,paint);paint.color=0x66FFFFFF;c.drawOval(x-r*.52f,y-r*.55f,x-r*.18f,y-r*.18f,paint);paint.color=0xFF6B4E9B.toInt();val p=Path();p.moveTo(x-r*.38f,y-r*.68f);p.lineTo(x,y-r*.9f);p.lineTo(x+r*.38f,y-r*.68f);p.lineTo(x+r*.12f,y-r*.42f);p.lineTo(x-r*.12f,y-r*.42f);p.close();c.drawPath(p,paint)}
    ...
    private fun drawCookie(c:Canvas,x:Float,y:Float,r:Float){c.drawCircle(x,y,r*.86f,paint);paint.color=0x55FFFFFF;c.drawOval(x-r*.48f,y-r*.52f,x-r*.14f,y-r*.18f,paint);paint.color=0xFF704A36.toInt();for(i in 0..5){val a=i*Math.PI/3;c.drawCircle(x+(kotlin.math.cos(a)*r*.45f).toFloat(),y+(kotlin.math.sin(a)*r*.45f).toFloat(),r*.09f,paint)}}
