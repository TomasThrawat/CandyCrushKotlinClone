...
class GameView(context: Context) : View(context) {
    ...
    private fun game(c:Canvas){text(c,"LEVEL $level",width*.16f,height*.06f,width*.043f,0xFF7B3F98.toInt(),true);text(c,"$score/$target",width*.50f,height*.06f,width*.038f,0xFF4D4147.toInt(),true);text(c,"MOVES $moves",width*.83f,height*.06f,width*.038f,0xFF4D4147.toInt(),true);val top=height*.12f;val cell=min(width*.112f,height*.68f/N);val left=(width-cell*N)/2f;val prog=((System.currentTimeMillis()-anim)/140f).coerceIn(0f,1f);for(r in 0 until N)for(col in 0 until N){val x=left+col*cell;val y=top+r*cell;box(c,x+1,y+1,x+cell-1,y+cell-1,if(look=="SNOW")0xFFDDECF5.toInt() else 0x22FFFFFF.toInt());if(board[r][col]>=0){var dx=0f;var dy=0f;if(prog<1f&&r==ar&&col==ac){dx=(bc-ac)*cell*prog;dy=(br-ar)*cell*prog};if(prog<1f&&r==br&&col==bc){dx=(ac-bc)*cell*prog;dy=(ar-br)*cell*prog};if(shadows&&graphics!="LOW"){paint.color=0x33000000;c.drawCircle(x+cell/2+dx+3,y+cell/2+dy+4,cell*.35f,paint)};candy(c,x+cell/2+dx,y+cell/2+dy,cell*.35f,board[r][col])}};btn(c,.04f,.83f,.30f,.92f,"HAMMER $hammer");btn(c,.35f,.83f,.65f,.92f,"SHUFFLE $shuffle");btn(c,.70f,.83f,.96f,.92f,"MENU");if(score>=target||moves<=0){paint.color=0xEE24152F.toInt();c.drawRect(0f,height*.30f,width.toFloat(),height*.68f,paint);text(c,if(score>=target)"LEVEL COMPLETE" else "OUT OF MOVES",width/2f,height*.43f,width*.06f,Color.WHITE,true);text(c,if(score>=target)"+"+reward()+" COINS" else "TRY AGAIN",width/2f,height*.51f,width*.045f,Color.WHITE,true);btn(c,.2f,.57f,.8f,.65f,if(score>=target)"CONTINUE" else "RETRY")}}
    ...
    private fun gameTap(x:Float,y:Float){if(score>=target||moves<=0){if(y in height*.57f..height*.65f){if(score>=target)complete()else start(level)};return};if(y>height*.83f){when{x<width*.32f->useH();x<width*.68f->useS();else->screen=Screen.MENU};return};val top=height*.12f;val cell=min(width*.112f,height*.68f/N);val left=(width-cell*N)/2;val col=((x-left)/cell).toInt();val row=((y-top)/cell).toInt();if(row !in 0 until N||col !in 0 until N)return;val dx=x-downX;val dy=y-downY;if(abs(dx)<32&&abs(dy)<32){sr=row;sc=col;return};if(sr<0){sr=row;sc=col;return};var tr=sr;var tc=sc;if(abs(dx)>abs(dy))tc+=if(dx>0)1 else -1 else tr+=if(dy>0)1 else -1;if(tr in 0 until N&&tc in 0 until N)move(sr,sc,tr,tc);sr=-1;sc=-1}
